...
import { addRecipe, getRecipes } from '@/features/post-place/lib/recipes';
import type { Recipe, Recipes } from '@/features/post-place/types/types';
...
function PostPlace() {
  ...
  const handleAdd = async (formData: FormData) => {
    ...
    return (
      ...
      <form action={handleAdd} className={tm('mb-[32px]')}>
        <label className="sr-only">카드 검색</label>
        <div className={tm('flex','border-black', 'border-[1px]', 'h-[40px]','rounded-lg', 'px-[12px]')}>
          <input type="text" name="recipe"  placeholder='원하는 지역을 검색해 보세요 ' className={tm('w-full')}/>
          <button
            type="submit"
            title="검색"
            className={tm(
        ...
        {data?.recipes?.map((item: Recipe) => (
          ...
          >
            <div className={tm('flex', 'gap-[7px]')}>
              ...
              <span className={tm('text-xs', 'text-white')}>{item.name}</span>
            </div>
